...
function MetricCard({ metric }: { metric: PolicyMetricDTO | { metricName: string; value: number; severity: SeverityLevel; description: string } }) {
  ...
  return (
    <div style={{ padding: '1.25rem', border: `2px solid ${colors.border}`, borderRadius: 12, background: colors.bg }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
        <h4 style={{ margin: 0, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#374151' }}>
          {metric.metricName.replace(/_/g, ' ')}
        </h4>
        <span style={{ padding: '0.15rem 0.5rem', borderRadius: 20, fontSize: '0.65rem', fontWeight: 700, color: '#fff', background: colors.badge, textTransform: 'uppercase' }}>
      ...
      <p style={{ fontSize: '0.78rem', color: '#6b7280', margin: '0.4rem 0 0', lineHeight: 1.4 }}>{metric.description}</p>
    </div>
  )
}
              ...
              {comparisons.map((c) => {
                ...
                return (
                  <td key={c.country} style={{ textAlign: 'center', padding: '0.6rem 0.5rem' }}>
                    ...
                    <br />
                    <span style={{ fontSize: '0.65rem', padding: '0.1rem 0.4rem', borderRadius: 10, background: colors.badge, color: '#fff' }}>
...
function LoadingSkeleton() {
  return (
    <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
      {[1, 2].map((i) => (
        <div key={i} style={{ padding: '1.5rem', border: '2px solid #e5e7eb', borderRadius: 12, background: '#f9fafb', animation: 'pulse 2s infinite' }}>
...
export default function DashboardPage() {
  ...
  const fetchComparison = useCallback(() => {
    ...
    fetch(`${API_URL}/api/policy/compare`, {
      ...
      body: JSON.stringify({
        policy: { id: `${selectedDomain}-analysis`, title: `${selectedDomain} Analysis`, description: '', domain: selectedDomain, actors: [], objectives: [] },
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 1060, margin: '0 auto', padding: '1.5rem' }}>
        ...
        <header style={{ marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <div>
              ...
              <p style={{ margin: '0.2rem 0 0', color: '#6b7280', fontSize: '0.85rem' }}>An\u00e1lisis de Pol\u00edticas P\u00fablicas</p>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {(['single', 'compare'] as ViewMode[]).map((mode) => (
                <button key={mode} onClick={() => setViewMode(mode)}
                  style={{ padding: '0.4rem 0.9rem', border: '1px solid #d1d5db', borderRadius: 8, background: viewMode === mode ? '#3b82f6' : '#fff', color: viewMode === mode ? '#fff' : '#374151', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600 }}>
                  {mode === 'single' ? 'Pa\u00eds' : 'Comparar'}
                </button>
              ))}
            </div>
          </div>
          <div style={{ height: 3, background: 'linear-gradient(to right, #3b82f6, #8b5cf6)', borderRadius: 2, marginTop: '0.75rem' }} />
        </header>
        ...
        <section style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          {viewMode === 'single' && (
            <select value={selectedCountry} onChange={(e) => setSelectedCountry(e.target.value)}
              style={{ padding: '0.45rem 0.75rem', border: '1px solid #d1d5db', borderRadius: 8, fontSize: '0.85rem', background: '#fff' }}>
          ...
          <button onClick={viewMode === 'single' ? fetchSingle : fetchComparison}
            style={{ padding: '0.45rem 1rem', border: '1px solid #3b82f6', borderRadius: 8, background: '#3b82f6', color: '#fff', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600 }}>
            Evaluar
          </button>
        </section>
        ...
        {error && (
          <div style={{ padding: '1rem', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 12, marginBottom: '1rem' }}>
        ...
        {viewMode === 'single' && data && !loading && (
          <>
            <section style={{ padding: '1rem', background: '#f8fafc', borderRadius: 12, border: '1px solid #e2e8f0', marginBottom: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
                <div>
                  ...
                  <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '0.82rem' }}>
                    {data.context.country} ({data.context.year}) &middot; <strong style={{ textTransform: 'capitalize' }}>{data.policy.domain}</strong>
                  </p>
                </div>
                <div style={{ display: 'flex', gap: '0.3rem' }}>
                  {data.context.sources.map((s) => (
                    <span key={s} style={{ padding: '0.15rem 0.45rem', background: '#e0e7ff', color: '#3730a3', borderRadius: 6, fontSize: '0.65rem', fontWeight: 600 }}>{s}</span>
                  ))}
                </div>
              </div>
            </section>
            <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
              {data.metrics.map((m) => <MetricCard key={m.metricName} metric={m} />)}
            </div>
          </>
        )}
          ...
          <>
            <section style={{ padding: '1rem', background: '#f8fafc', borderRadius: 12, border: '1px solid #e2e8f0', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', fontSize: '0.82rem', color: '#64748b' }}>
                <span>Pa\u00edses: <strong>{compareData.summary.countriesAnalyzed}</strong></span>
                <span>Mejor: <strong style={{ color: '#16a34a' }}>{compareData.summary.bestPerforming}</strong></span>
                <span>Peor: <strong style={{ color: '#dc2626' }}>{compareData.summary.worstPerforming}</strong></span>
                <span>Mayor varianza: <strong style={{ textTransform: 'capitalize' }}>{compareData.summary.highestVarianceMetric.replace(/_/g, ' ')}</strong></span>
              </div>
            </section>
        ...
        {/* Footer */}
        <footer style={{ marginTop: '1.5rem', paddingTop: '0.75rem', borderTop: '1px solid #e5e7eb', fontSize: '0.75rem', color: '#9ca3af', display: 'flex', justifyContent: 'space-between' }}>
          <span>HodioPolitica v1.0.0</span>
          <span>{viewMode === 'single' && data ? new Date(data.evaluatedAt).toLocaleString('es-ES') : viewMode === 'compare' && compareData ? new Date(compareData.evaluatedAt).toLocaleString('es-ES') : ''}</span>
        </footer>
      </main>
    </>
  )
